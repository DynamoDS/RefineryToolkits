...
namespace Autodesk.GenerativeToolkit.Utilities.GraphicalGeometry
{
    ...
    public static class Curves
    {
        ...
        internal static bool DoesIntersect(Autodesk.DesignScript.Geometry.Line line1, Autodesk.DesignScript.Geometry.Line line2)
        {
        ...
        internal static bool DoesIntersect(Autodesk.DesignScript.Geometry.Line line, DSPoint point)
        {
        ...
        public static Dictionary<string, object> BuildPolygons(List<Autodesk.DesignScript.Geometry.Line> lines)
        {
            ...
            List<Autodesk.DesignScript.Geometry.Line> dsLines = new List<Autodesk.DesignScript.Geometry.Line>();
            ...
            foreach (gPolygon gP in gPolygons)
            {
                ...
                else if (gP.Edges.Count > 1)
                {
                    foreach(gEdge edge in gP.Edges)
                    {
                        ...
                        dsLines.Add(Autodesk.DesignScript.Geometry.Line.ByStartPointEndPoint(start, end));
                    }
                }
                else
                {
        ...
        public static object Polygonize(DSCurve curve, double maxLength, bool asPolycurve = false)
        {
            ...
            else
            {
                ...
                if(divisions > 1)
                {
                    ...
                    lines.Add(Autodesk.DesignScript.Geometry.Line.ByStartPointEndPoint(curve.StartPoint, points.First()));
                    for (var i = 0; i < points.Count() - 1; i++)
                    {
                        lines.Add(Autodesk.DesignScript.Geometry.Line.ByStartPointEndPoint(points[i], points[i + 1]));
                    }
                    lines.Add(Autodesk.DesignScript.Geometry.Line.ByStartPointEndPoint(points.Last(), curve.EndPoint));
                }
                else
                {
                    lines.Add(Autodesk.DesignScript.Geometry.Line.ByStartPointEndPoint(curve.StartPoint, curve.EndPoint));
                }
            }
